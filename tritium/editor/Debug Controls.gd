...
func _ready() -> void:
    var data = editor.mech_data
    add_controls_for_properties(data, self)
...
func create_control_for_property(property: Dictionary, data: Object) -> Control:
    match property.type:
        ...
        TYPE_OBJECT:
            var sub_data = data.get(property.name)
            if sub_data is MechData.Data:
                var object_container = VBoxContainer.new()
                add_controls_for_properties(sub_data, object_container)
